...
void test_BFS() {
    ...
    for (int x = 0; x < 5; x++) {
    game.board->cells[x] = CELL_WALL;
	game.board->cells[24 - x] = CELL_WALL;
	}
    game.board->cells[5] = CELL_WALL;
	game.board->cells[9] = CELL_WALL;
	game.board->cells[10] = CELL_WALL;
	game.board->cells[14] = CELL_WALL;
    game.board->cells[15] = CELL_WALL;
    game.board->cells[19] = CELL_WALL;
    game.board->cells[17] = CELL_FOOD;
    int* path = BFS(game, 4);
    assert(path[0] == -2);
    path = BFS(game, 0);
    assert(path[0] == -2);
