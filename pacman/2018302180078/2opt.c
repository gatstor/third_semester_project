...
int main() {
	...
	for(int j=0;j<top-1;j++)
	{
		...
		douzi_length+=cul_length(cul_path_douzi);
		cul_path_douzi=Dijkstra(G,douzi_pos[j],douzi_pos[j+1]);
		douzi_length+=cul_length(cul_path_douzi);*/
	}
	int tmp_length;
	int first=0;
	for(int i=0;i<10000;i++)
	{
		...
		/*else if(k>0&&k<top-1&&l>0&&l<top-1&&first&&((k-l)>2||(l-k)>2))
		{
			tmp_length=temp_length;

			cul_path_temp=Dijkstra(G,temp[k],temp[k+1]);
			tmp_length-=cul_length(cul_path_temp);

			cul_path_temp=Dijkstra(G,temp[k-1],temp[k]);
			tmp_length-=cul_length(cul_path_temp);

			cul_path_temp=Dijkstra(G,temp[l],temp[l+1]);
			tmp_length-=cul_length(cul_path_temp);

			cul_path_temp=Dijkstra(G,temp[l-1],temp[l]);
	   		tmp_length-=cul_length(cul_path_temp);


			cul_path_temp=Dijkstra(G,temp[l],temp[k+1]);
			tmp_length+=cul_length(cul_path_temp);

			cul_path_temp=Dijkstra(G,temp[k-1],temp[l]);
			tmp_length+=cul_length(cul_path_temp);

			cul_path_temp=Dijkstra(G,temp[k],temp[l+1]);
			tmp_length+=cul_length(cul_path_temp);

			cul_path_temp=Dijkstra(G,temp[l-1],temp[k]);
			tmp_length+=cul_length(cul_path_temp);
			if(temp_length < douzi_length)
			{
			 ...
			 douzi_pos[l] = swap;
 			 douzi_length = tmp_length;
		 	 temp_length = tmp_length;
		 	 times=0;			 
			}
			else
				times++;
		...
		else
		{
			...
			{	if(j == 0)
				{
					/* cul_path_temp=Dijkstra(G,start,temp[0]);
	           		 temp_length+=cul_length(cul_path_temp);*/
					temp_length+=length_head[temp[0]];
				}
				/*cul_path_temp=Dijkstra(G,temp[j],temp[j+1]);
				temp_length+=cul_length(cul_path_temp);*/
	...
	//zai douzi_pos lim cunru douzi shunxu
	cul_path_douzi=Dijkstra(G,start,douzi_pos[0]);
